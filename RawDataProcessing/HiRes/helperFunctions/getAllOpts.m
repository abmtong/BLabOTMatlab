function outOpts = getAllOpts(structname, fncnames)
...
if nargin < 1 || isempty(structname)
    structname = 'opts';
end
if nargin < 2 %pick from filepicker
    [files, path] = uigetfile('*.m', 'Choose your code files that use the opts struct', 'MultiSelect','on');
    if ~iscell(files)
        files = {files};
    end
    filepaths = cellfun(@(x) [path x], files, 'Uni', 0);
else %fnc name(s) specified, so create struct
    if ~iscell(fncnames)
        fncnames = {fncnames};
    end
    filepaths = cellfun(@(x) which(x), fncnames, 'Uni', 0);
end
fnames = [];
defaults = [];
