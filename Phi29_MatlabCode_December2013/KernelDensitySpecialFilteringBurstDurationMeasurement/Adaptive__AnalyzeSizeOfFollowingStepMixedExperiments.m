function [Single10bpCycle TwoStep10bpCycle TwoStep20bpCycle ThreeStep20bpCycle ThreeStep30bpCycle IncompleteCycles NewDistribution] = Adaptive__AnalyzeSizeOfFollowingStepMixedExperiments(StructFC,StructBursts)
...
%    global analysisPath; % reads where are the files
    ...
    Cummulative=zeros(1,20/1);
    for r=1:length(StructBursts)
        Bursts.Size=StructBursts(r).Size;
        FC=StructFC{r};
   ...
   % new=new./sum(new);
    ...
    close all;
    figure;
    plot(x,(new+f)/sum(new+f),'LineWidth',4,'Color',[0.5 0.5 0.5]);
    Cummulative=(Cummulative + (new+f)/sum(new+f));
    pause(1);
    display(r)
    end
    ...
    close all;
    Cummulative=Cummulative/sum(Cummulative);
    figure;
    plot(x,Cummulative,'LineWidth',4,'Color',[1 0 0])
    hold on;
end