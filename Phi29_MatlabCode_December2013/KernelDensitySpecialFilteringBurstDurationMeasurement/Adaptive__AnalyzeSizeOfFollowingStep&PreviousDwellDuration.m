function [StructureDwells StructureBursts] = Adaptive__AnalyzeSizeOfFollowingStepPreviousDwellDuration(FC,Dwells,Bursts)
...
%    global analysisPath; % reads where are the files
    ...
    StructureDwells(1)=TempDwells(1:FC(1));
    for k=2:length(FC)
        CycleNumber=sum(FC(1:k-1));
        StructureDwells(FC)=TempDwells(CycleNumber+1:CycleNumber+1+FC(k));  
        StructureBursts(FC)=TempBursts(CycleNumber+1:CycleNumber+1+FC(k));
    end
