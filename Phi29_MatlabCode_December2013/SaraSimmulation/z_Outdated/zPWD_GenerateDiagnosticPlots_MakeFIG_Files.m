function PWD_GenerateDiagnosticPlots_MakeFIG_Files(FeedbackCycles)
...
File = uigetfile([ [analysisPath '\'] 'phage*.mat'], 'Please Selecte Phage Traces for PWD Diagnostics:','MultiSelect', 'off');
...
for p=1:length(File) %p is the phage index
    ...
    for fci = 1:length(FeedbackCycles); %fc is the index of feedback cycles
        fc  = FeedbackCycles(fci);
        %use boxcar filter instead of filter and decimate
        %T   = FilterAndDecimate(phage.time{fc},Filter); %#ok<*AGROW>
        %L   = FilterAndDecimate(phage.contour{fc},Filter);
        T   = BoxcarFilter(phage.time{fc},Filter); %#ok<*AGROW>
        ...
        if range(T)>1 %only look at the feedback traces longer than 1 sec
            ...
            %set(gca,'XLim',[0 100]); %for consistency and ease of review
            xlabel('Distance (bp)');
            ...
            saveas(h, [ImgFolder '\' phage.file(1:end-4) '_' num2str(fc)], 'fig');
            close(h);
        else
