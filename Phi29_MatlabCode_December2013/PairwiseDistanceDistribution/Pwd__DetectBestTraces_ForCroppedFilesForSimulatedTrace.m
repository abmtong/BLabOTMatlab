function BestDataInd = Pwd__DetectBestTraces_ForCroppedFiles()
...
%
% USE: BestDataInd = Pwd_DiagnoseCroppedTraces()
%
...
Prompt = {'Data Acquisition Bandwidth (Hz)', ...
          ...
          'Display Distance Range 0-xx bp'    , ...
          'PWD Contrast Threshold (>1.3 works well)'};
Title = 'Enter the Following Parameters';
Lines = 1;
Default = {'2500','100','0.5','40','1.2'};
Options.Resize      = 'on'; 
...
FilterFactor  = round(Bandwidth/F); %filtering factor
...
for p=1:length(File) %p is the phage index
    ...
    if ~exist(CropFile,'file') %if the crop file doesn't exist
        ...
        for fc_index=1:length(SelectedFeedbackCycles); %fc is the index of feedback cycles
            ...
            T  = Pwd_BoxcarFilter(phage.time{fc},    FilterFactor); %do a boxcar filter for smoother PWD
            L  = Pwd_BoxcarFilter(phage.contour{fc}, FilterFactor);
            ...
            if range(T)>0.5 %only look at the feedback traces longer than 0.5 sec
                ...
                if ~isempty(Ind)
                    ...
                    ValleyInd0 = D>1  & D<9;  % look for the zeroth valley
                    ...
                    if strcmp(Verdict,'Pass')
                        ...
                        if ~strcmp(BestDataInd(end).PhageTrace,File{p})
                            ...
                            %the current PhageTrace already exists in the list, just add the FC
                            BestDataInd(end).FeedbackCycle(end+1) = fc;
                        end
                    else
                        %the current FC is not good enough, just plot the PWD
                    ...
                    set(gca,'XLim',[0 DistanceRange],'YLim',YLim,'Box','on');
                    xlabel('Distance (bp)'); ylabel('Normalized PWD');
                    Coverage = [num2str(round(min(L))) '-' num2str(round(max(L))) 'bp'];
                    title(['Simulated trace, Feedback Cycle: ' num2str(fc) ', Coverage: ' Coverage]);
                    ...
                    if strcmp(Verdict,'Pass') %save good trace images into the "Good" folder
                        ...
                        if ~isdir(ImgFolder); mkdir(ImgFolder); end;
                        saveas(h, [ImgFolder filesep 'Simulated trace _' num2str(fc)], 'png');
                    ...
                    disp(['Feedback Trace #' num2str(fc) ' of  Simulated trace  was skipped, does not cover enough']);
                end
            else
                disp(['Feedback Trace #' num2str(fc) ' of Simulated trace was skipped due to insufficient data.']);
            end   
        end
        ElapsedT=toc(StartT);
        disp(['It took ' num2str(ElapsedT) ' sec to generate PWD Diagnostic Plots for Simulated trace']);
    end
end
...
choice = questdlg(qstring,'Save File?','Yes, Save Index','No',options);
    if  strcmp(choice,'Yes, Save Index');
        [IndexFile,IndexFilePath] = uiputfile([analysisPath filesep 'BestPwdScore_.txt'],'Save the Index of the Best Feedback Cycles');
