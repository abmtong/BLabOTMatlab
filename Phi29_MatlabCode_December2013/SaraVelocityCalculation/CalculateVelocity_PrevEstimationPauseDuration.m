function CalculateVelocity_PrevEstimationPauseDuration()
...
% Gheorghe Chistol, 13 Oct 2012
    ...
    ForceBoundaries   = [4:1:12]; %force intervals, either in 1 or 2 pN intervals is ok
    ...
    MinSlipSize       = 5; %in bp
    MaxTimeSpan       = 15; %if a feedback cycle is longer than MaxTimeSpan seconds, break it up this helps makes step-finding much faster
    ...
    for f = 1:length(FileName)
        ...
        if ~exist(CropFile,'file') %do not process trace if crop file doesn't exist
            ...
            fclose(FID);
            %Calculate Pause Threshold based on the trace that is being
            %analyzed
            %[MinPauseDur StepFindPenalty]=CalculatePauseThresholdBasedOnVelocity(Trace,Tstart,Tstop);
            %disp(StepFindPenalty);
            ...
            for fc = 1:length(Trace.time) % 'fc' is the index of the feedback cycle
                ...
                disp(['This is FC =' num2str(fc)])  ;  
                    ...
                    [Data(ind).Dwells Data(ind).LadderTime Data(ind).LadderContour Data(ind).LadderForce] = ...
                        CalculateVelocity_OrganizeDwellDataCorrection(DwellInd,Data(ind).FiltTime,Data(ind).FiltForce,MinPauseDur,NumFrag);

                    [Data(ind).Slips Data(ind).Pauses Data(ind).SlipPauseFreeSegments Data(ind).DwellPrecedingSlips] = ...
                    ...
                    Data(ind).Velocity = CalculateVelocity_DivideAndConquer_MaxTimeSpan(Data(ind),ForceBoundaries,VelFiltFact,MinContSpan,MinNumPts,PlotOption,PlotSave); %#ok<*AGROW>
            ...
            DiagPlotFilePath = [Data(1).FilePath filesep 'VelocityCalculation_AdaptivePauseThreshold'];
            DiagPlotFileName = ['VelocityDiagnostics_AdaptivePauseThreshold' Data(1).FileName(1:end-4) '.png'];
            ...
            SavePath = [Data(ind).FilePath filesep 'VelocityCalculation_AdaptivePauseThreshold'];
            SaveFile = ['VelocityResults_AdaptivePauseThreshold_' Data(ind).FileName(1:end-4) '.mat'];
