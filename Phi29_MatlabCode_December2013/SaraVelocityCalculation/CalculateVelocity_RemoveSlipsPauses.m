function [NoPauses Slips Pauses SlipPauseFreeSegments] = CalculateVelocity_RemoveSlipsPauses(Data,MinPauseDur,MinSlipSize)
...
% Gheorghe Chistol, 29 Feb 2012
    ...
    Slips.SlipSize           = [];
    NoPauses                 = 0;
    MaxPauseSeparationInCluster = 50;
    ...
    for p = 1:length(PauseInd)
        ...
        if p == 1
           ...
           if Pauses.PauseLocation(p-1) - Pauses.PauseLocation(p) > MaxPauseSeparationInCluster;
                NoPauses = NoPauses+1;
           end
        end 
    end
