...
% Gheorghe Chistol, 09 Feb 2012
    ...
    if sDirection % direction=1 for x, direction=0 for y
        ...
        %A: filters [time-moving average, size nWindow steps] and
        %finds the indices where the filtered curve moves less than
        %vThreshold; this array of indices is idx
        startInd = [idx(1); idx(find(diff(idx) > 5) + 1)];
        %A: Identify burst ends by areas of adjacent movement > vThreshold (any
        %run where the curve changes at least every one in five points
        %counts); these are the start indices
        endInd   = [idx(find(diff(idx) > 5)); idx(end)]; %#ok<*FNDSB>`
        %the end indicies. Each dwell-burst is then data(startInd(i):endInd(i)).
        %somewhere these went from row to column vectors? (; delimiters)
    ...
    endAll = endInd;
    ind = find((startAll <= endAll) == 1); %need since frontPad might push the start of the dwell past the end of the burst?
