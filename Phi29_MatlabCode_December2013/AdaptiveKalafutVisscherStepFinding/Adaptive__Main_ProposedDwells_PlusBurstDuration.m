function Adaptive__Main_ProposedDwells_PlusBurstDuration()
    ...
    Default       = {'2500','See the code','3','2','1.2','1000'};
    ...
    BandwidthListFast = [60 80 100]; %for translocation faster than 10 bp/s % check Adaptive__Main for the non-modified version of this script
    BandwidthListSlow = [25]; %for translocation slower than 10bp/s
    ...
    [IndexFile IndexFilePath] = uigetfile([ [analysisPath filesep] '*.txt'], 'Please select the Index File','MultiSelect', 'off');
                    IndexFile = [IndexFilePath filesep IndexFile]
                     SaveFile = [IndexFile(1:end-4) '__PF_' num2str(PenaltyFactor) '_ProposedBursts_September2015.mat']; %where the step-finding results will be saved
        ...
        if ~exist(CurrentPhageFile,'file') %if the phage data file does not exist
            ...
            for fc = 1:length(SelectedFeedbackCycles{p}) %fc is the index of the "FeedbackCycle"
                ...
                if range(RawT)<0.25 %the trace needs to have more than 0.25 sec worth of data 
                    ...
                    disp(['+ Analysing Feedback Cycle ' num2str(CurrentFeedbackCycle)]);
                    disp(['HI']);
                    ...
                    for s = 1:length(IndexStart) %run a separate cycle for each analysis section 
                        for b = 1:length(BandwidthList) %run a separate cycle for each bandwidth
                            ...
                            DataSet = FinalDwells{p}{fc}{s}{b};
                            if length(DataSet.DwellTime)>1 %need at least two valid dwells for this
                                FinalDwells{p}{fc}{s}{b}.tTestTime   = tTime;
                                FinalDwells{p}{fc}{s}{b}.tTestCont   = tCont;
                                FinalDwells{p}{fc}{s}{b}.tTestLogSgn = log(tSgn');
                                FinalDwells{p}{fc}{s}{b}.BurstStartTime    = NaN*FinalDwells{p}{fc}{s}{b}.StepSize;%there are no burst durations yet
                                FinalDwells{p}{fc}{s}{b}.BurstFinishTime   = NaN*FinalDwells{p}{fc}{s}{b}.StepSize;%there are no burst durations yet
                                FinalDwells{p}{fc}{s}{b}.BurstLogSgnCutoff = NaN*FinalDwells{p}{fc}{s}{b}.StepSize;%there are no burst durations yet
                                FinalDwells{p}{fc}{s}{b}.BurstDuration     = NaN*FinalDwells{p}{fc}{s}{b}.StepSize;%there are no burst durations yet
                                    ...
                                    if DataSet.Start(d+1)-DataSet.Finish(d) == 1 %i.e. if dwell d and d+1 are consecutive
                                        ...
                                        if length(MinInd)==1 %precisely one local minimum point found
                                            ...
                                            if ~isnan(BackTime) && ~isnan(ForwTime) % succesfully found a burst duration
                                                FinalDwells{p}{fc}{s}{b}.BurstDuration(d) = ForwTime-BackTime; %burst Duration between these two dwells
                                                FinalDwells{p}{fc}{s}{b}.BurstStartTime(d) = BackTime;
                                                FinalDwells{p}{fc}{s}{b}.BurstFinishTime(d) = ForwTime;
                                                FinalDwells{p}{fc}{s}{b}.BurstLogSgnCutoff(d) = Cutoff;
                                            end
                                        end
                                    end
                                end
                                ...
                                for BurstInd = 1:length(FinalDwells{p}{fc}{s}{b}.BurstStartTime)
                                    StartTime  = FinalDwells{p}{fc}{s}{b}.BurstStartTime(BurstInd);
                                    FinishTime = FinalDwells{p}{fc}{s}{b}.BurstFinishTime(BurstInd);
                                    Cutoff     = FinalDwells{p}{fc}{s}{b}.BurstLogSgnCutoff(BurstInd);
                                ...
                                ImageFolderName=[analysisPath filesep 'AdaptiveStepFinding_BurstDetection_Threshold_September2015']; %Save the current figure as an image in a folder for later 
