function [RecordOfValidatedDwells ProposedDwells ValidatedDwells LocalMaxima]= Adaptive_ValidateDwells_FragDwellInd_Validate(CandidateDwells, LocalMaxima, MaxSeparation)
    ...
    RecordOfValidatedDwells = []; %Stores 1 if the dwell
    ...
    for cd = 1:length(CandidateDwells)
        ...
        if NearestPeakDist < MaxSeparation
            ...
            RecordOfValidatedDwells(cd)=1;
        else
            RecordOfValidatedDwells(cd)=0;
        end
        ProposedDwells(cd)= CandidateDwells(cd);
    end
 ...
 %               if ValidDwellPerPeakCount(unpi)==2 %we got two valid
 %               dwells for this one peak
...
%                          ValidatedDwells(d1).Mean              = mean(ValidatedDwells(d1).Start:ValidatedDwells(d1).Finish);
