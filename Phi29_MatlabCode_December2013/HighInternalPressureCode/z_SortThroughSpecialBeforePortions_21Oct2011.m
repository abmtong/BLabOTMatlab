function z_SortThroughSpecialBeforePortions_21Oct2011()
% PauseClusters{}{}()
%                         StartDwellInd: 10
%                        FinishDwellInd: 11
%                           ClusterSpan: 11.4395
...
%                          BeforeIsGood: 1
%                            BeforeData: [1x1 struct]
%
% Gheorghe Chistol, 21 Oct 2011
    
addpath([pwd filesep 'KalafutVisscher_StepFinding' filesep], '-end'); %where the KV scripts are located
global analysisPath;
[DataFile DataPath] = uigetfile([ [analysisPath filesep ] '*.mat'], 'Please select the Step Finding Results file','MultiSelect', 'off');
    ...
    for fc=1:length(PauseClusters{ph}) %fc is the FeedbackCycle index
        if ~isempty(PauseClusters{ph}{fc})
            for c=1:length(PauseClusters{ph}{fc}) %c is the Cluster index
                if isfield(PauseClusters{ph}{fc}(c), 'BeforeIsGood')
...
save([DataPath filesep DataFile(1:end-4) '_Valid.mat'],'FinalDwells','PauseClusters','CumulativeValidData');
