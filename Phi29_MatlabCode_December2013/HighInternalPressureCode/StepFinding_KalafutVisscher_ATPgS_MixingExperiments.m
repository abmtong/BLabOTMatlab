function StepFinding_KalafutVisscher_ATPgS_MixingExperiments(Vel)
    ...
    SaveFile                  = [IndexFile(1:end-4) '_ResultsKV_ MinPauseDur_Variable.mat']; %where the step-finding results will be saved
    ...
    for p=1:length(SelectedPhages) %index p stands for "phage" 
        ...
        if ~exist(CurrentPhageFileName,'file') %if the phage data file does not exist
            ...
            PauseClusters{p} = KV_ATPgS_IdentifyPauseClusters2(FullFinalDwells,FigureH,MaxSeparation,(1./Vel{p})*100);
            
            %save the figure
            figure(FigureH);
            title(['File: ' PhageData.file(1:end-4) ', SIC Penalty: ' num2str(PenaltyFactor)]);
            FolderName=['ATPgS_PauseClusterFinding_MinPauseDur_Variable']
