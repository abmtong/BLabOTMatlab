function StepFinding_ProcessTrace_FIG()
% 
...
Lines = 1;
Default = {'20','2500','5','1','0.1','2','0.2','0.005'};
        ...
        for fc=1:length(SelectedFeedbackCycles{p}) %index fc stands for "FeedbackCycle"
            ...
            t = PhageData.time{SelectedFeedbackCycles{p}(fc)};
                ...
                for s=1:length(Dwells{end}.mean)-1 %we can't calculate the step-size after the last dwell
                 ...
                 plot(PhageData.timeFiltered{CurrentFeedbackCycle}, PhageData.contourFiltered{CurrentFeedbackCycle},'Color',[0.8 0.8 0.8]);
                 grid off;
                 %Plot the Stepping
                 ...
                 ImageFileName = [ImageFolderName '\' PhageData.file(1:end-4) '_' num2str(CurrentFeedbackCycle) '.fig'];
