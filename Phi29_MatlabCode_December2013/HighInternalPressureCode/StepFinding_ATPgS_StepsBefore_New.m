function StepFinding_ATPgS_StepsBefore_New()
    ...
    for df=1:length(DataFile)
                ...
                if ~isempty(PauseClusters{ph}{fc})
                    for c=1:length(PauseClusters{ph}{fc}) %c is the Cluster index
                        ...
                        if strcmp(reply,'b')
                            ...
                            SaveFolder = [analysisPath filesep 'BeforeStepCensus'];
                            if ~isdir([analysisPath filesep 'BeforeStepCensus'])
                                mkdir(SaveFolder);
                            end
        ...
        save([DataPath filesep DataFile{df}(1:end-4) '_BeforeExtra.mat'],'PauseClusters','FinalDwells');
        figure('Units','normalized','Position',[0.1603 0.3828 0.2284 0.2695]);
        area(KernelX,KernelY,'FaceColor','m','LineWidth',1);
