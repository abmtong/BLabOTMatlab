function PlotMultipleCroppedTraces(F, TimeOffset)
...
PlotCount=4; %number of traces plotted
...
for i=1:length(File)
    ...
    if exist(CropFile,'file') %check if the crop file exists, if it doesn't, don't process the phage file at all
        ...
        if PlotCount<=length(TimeOffset)
            ...
            TextX=double(Time(1)+TimeOffset(PlotCount));
        else
            plot(Time, Contour,'b');
            TextY=double(Contour(1));
            TextX=double(Time(1));
        ...
        %Text
        Text=File{i}(6:end-4);
        text(TextX,TextY,Text,'FontSize',8);
        %set(h,'FontSize',8);
    else
