function Adaptive__Main_ProposedDwells_PlusBurstDuration()
    ...
    BandwidthListFast = [60 80 100 ]; %for translocation slower than 10 bp/s
    BandwidthListSlow = [60 80 100]; %for translocation faster than 10bp/s
    ...
    [IndexFile IndexFilePath] = uigetfile([ [analysisPath filesep] '*.txt'], 'Please select the Index File','MultiSelect', 'off');
                    IndexFile = [IndexFilePath filesep IndexFile];
                     SaveFile = [IndexFile(1:end-4) '_AdaptiveSicStepFinding_PF_' num2str(PenaltyFactor) 'AllFiles_ProposedBursts.mat']; %where the step-finding results will be saved
