function [Nmin DwellTimes] = ADPReleaseTitrationSimulatedExperiment()
...
%
    ...
    Tadp_off = [1/5 1/10 1/20 1/30 1/40 1/50]; % (kADP_off = 40 s^-1, 0.025)
    ...
    AlphaTb= 10;
    Tatp_tight = 1/5000; % (kATP_tight = 5000 s^-1, 0.0002)
    ...
    for t=1:length(Tadp_off)  % this cycle goes through all the ATP conditions that are defined in the above vector
        ...
        for v=1:length(ATP)
            ...
            [Nmin DwellTimes]=MonteCarlo_Simulating5Subunits(Tatp_on,Tatp_off,Tadp_on,Tadp_off(t),Tatp_tight,AlphaT,AlphaD,AlphaTb,Nrounds); 
    ...
    errorbar(1./Tadp_off,N_mean,N_error,'Marker','*','LineStyle',':','LineWidth',2,...
     'Color',[0.0431372549019608 0.517647058823529 0.780392156862745]);
    % Create xlabel
    xlabel('ADP release constant (1/s)','FontSize',20)
    ylabel('Nmin (a.u.)','FontSize',20); hold on;  
    ylim([2,8]);
    %set(gca,'xscale','log')
    %figure('Position',[750          200        600         400]);
    %errorbar(1./Tadp_off,Vel,Vel_error,'Marker','*','LineStyle',':','LineWidth',2,...
    %'Color',[0 0 1]);
    % Create xlabel
    ...
    % Create ylabel
    %ylabel('Velocity (bp/s)','FontSize',20); hold on;  
    
    figure('Position',[550          200        400         200]);
    plot(1./Tadp_off,Km,'Marker','*','LineStyle',':','LineWidth',2,...
    'Color',[0 0 1]);
    % Create xlabel
    xlabel('ADP release constant (1/s)','FontSize',20);
    hold on;
    ...
    plot(1./Tadp_off,Vmax,'Marker','*','LineStyle',':','LineWidth',2,...
    'Color',[0 0 1]);
    % Create xlabel
    xlabel('ADP release constant (1/s)','FontSize',20);
    hold on;
