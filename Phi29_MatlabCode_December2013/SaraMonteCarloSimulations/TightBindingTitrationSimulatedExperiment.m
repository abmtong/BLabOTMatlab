function [Nmin DwellTimes] = TightBindingTitrationSimulatedExperiment()
...
%
    ...
    ADP_on   = [1.1 2.2 4.0]; % (1.0 uM^-1 s^-1)
    Tatp_off = 0.05; % (kATP_off = 20 s^-1, 0.05) 
    Tadp_off = 1/90; % (kADP_off = 40 s^-1, 0.025)
    AlphaT = 10; % this means the special subunit has 10 times higher affinity for ATP than the rest of the subunits
    AlphaD = 5; % this means the special subunit releases ADP 10 slower than the rest of the subunits
    AlphaTb = 20; % this means the special subunit tightly-binds 20 times slower than regular subunits
    Tatp_tight = [1/1 1/10 1/30 1/50 1/100 1/250 1/500 1/1000 1/5000]; % (kATP_tight = 5000 s^-1, 0.0002)
    ...
    N_lower = [];
     ...
     close all;
     hax1=figure('Position',[100          200        600         400]);
     %hax1=axes;
     hax2=figure('Position',[550          200        400         200]);
     %hax2=axes;
     hax3=figure('Position',[1000          200        400         200]);
     %hax3=axes;
     
    for a=1:length(ADP_on)
      ...
      for t=1:length(Tatp_tight)  % this cycle goes through all the ATP conditions that are defined in the above vector
        ...
        for v=1:length(ATP)
            ...
            if ADP==0
                ...
                Tadp_on = 1/(ADP_on(a)*ADP); %the ADP binding time is inversely proportional to ADP concentration
            end
            
            [Nmin DwellTimes]=MonteCarlo_Simulating5Subunits(Tatp_on,Tatp_off,Tadp_on,Tadp_off,Tatp_tight(t),AlphaT,AlphaD,AlphaTb,Nrounds); 
