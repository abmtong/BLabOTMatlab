function outflag = fsHMMsave_4steps(infilepath, nitermax, stopatlogp)
%current: fsHMMV1b
...
while iter <= nitermax
    if iter == 1
        ...
        %do stepfinding
        try
            newres = findStepHMMV1b_4steps(tr, mod, verboseflag);
        catch
            newres = [];
            fcdata.hmm = newres;
        ...
        %do stepfinding
        try
            newres = findStepHMMV1b_4steps(tr, preres, 0);
        catch
            fcdata.hmmfinished = -1;
            outflag = -1;
            save(infilepath, 'fcdata')
