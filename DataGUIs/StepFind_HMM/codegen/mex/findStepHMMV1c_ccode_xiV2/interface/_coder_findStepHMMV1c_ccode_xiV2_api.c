/*
 * _coder_findStepHMMV1c_ccode_xiV2_api.c
 *
 * Code generation for function '_coder_findStepHMMV1c_ccode_xiV2_api'
 *
 */
...
static int32_T g_emlrt_marshallIn(const mxArray *maxwid2, const char_T
  *identifier);
static int32_T h_emlrt_marshallIn(const mxArray *u, const emlrtMsgIdentifier
  *parentId);
static real_T i_emlrt_marshallIn(const mxArray *sig, const char_T *identifier);
  ...
  *identifier)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = h_emlrt_marshallIn(emlrtAlias(maxwid2), &thisId);
  emlrtDestroyArray(&maxwid2);
  return y;
}
...
static real_T i_emlrt_marshallIn(const mxArray *sig, const char_T *identifier)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = j_emlrt_marshallIn(emlrtAlias(sig), &thisId);
...
void findStepHMMV1c_ccode_xiV2_api(const mxArray * const prhs[11], const mxArray
  *plhs[1])
{
  emxArray_real_T *tr;
  emxArray_real_T *al;
  ...
  e_emlrt_marshallIn(emlrtAlias(prhs[8]), "lb2", lb2);
  maxwid2 = g_emlrt_marshallIn(emlrtAliasP(prhs[9]), "maxwid2");
  sig = i_emlrt_marshallIn(emlrtAliasP(prhs[10]), "sig");

  /* Invoke the target function */
  findStepHMMV1c_ccode_xiV2(tr, al, a, y, be, lb, ub, wid, lb2, maxwid2, sig, xi);

  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(xi);
  xi->canFreeData = false;
  emxFree_real_T(&xi);
  lb2->canFreeData = false;
  emxFree_int32_T(&lb2);
...
/* End of code generation (_coder_findStepHMMV1c_ccode_xiV2_api.c) */
