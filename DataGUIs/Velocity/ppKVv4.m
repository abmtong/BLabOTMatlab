function out = ppKVv4(data, fdata, inOpts)
%V3: Modular now
% This just sorts into tloc and non-tloc sections
%V4: gets burst right before and after the bt
...
for i = 1:ntr
    ...
    while j < len-1 %Go until we run out of indSta or indEnd. Use while so we can 'skip ahead'
        ...
        ind = ind * opts.filwid{2}; %This will start at opts.filwid, shift to 1 below
    ...
    %Debug: Plot every nth trace. No plot if opts.verbose.trace = inf.
...
%Make tfbt logical, so we can use for array indexing
outtfbt = logical(outtfbt);

%Find the burst size just before + after bt events
ibt = find(outtfbt);
nbt = length(ibt);
bupre = zeros(1,nbt);
bupost= zeros(1,nbt);
for i = 1:nbt
    bupre(i) = outmea{ibt(i)-1}(end-1) - outmea{ibt(i)-1}(end);
    bupost(i)= outmea{ibt(i)+1}(1) - outmea{ibt(i)+1}(2);
end
...
out.bt = struct('tra', (outtra(outtfbt)), ...
                ...
                'bupre', num2cell(bupre), ...
                'bupost', num2cell(bupost));
