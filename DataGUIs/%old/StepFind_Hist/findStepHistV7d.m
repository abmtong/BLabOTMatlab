function [outInd, outMean, outS] = findStepHistV7d(inContour, inYRes, inNoise, p, inDec)
...
% (e) redacted: tried 'Bead Dynamics'- but response is so quick (response time ~ 1/Fc)
...
if nargin < 4 || isempty(p)
    ...
    W(1:hei+1:end) = 0; %Zero the diagonal
else
...
for i = 1:len-1
    ...
    yRange = ymin:ymax;
    [dJ, ddS] = min( bsxfun(@plus,J(yRange)',W(yRange,yRange)) ,[],1);
