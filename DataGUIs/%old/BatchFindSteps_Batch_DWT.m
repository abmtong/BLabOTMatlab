function outDwells = BatchFindSteps_Batch_DWT(filter, width, decimate, str, inFilePath, fileNames, inOptions, useExt)
    ...
    if ind == size(crops,1)+1
        ...
        for j = 1:length(con)
            ...
            if length(trace) > 100 && ~prod(isnan(trace)) %dont process empty / tiny / NaN traces
                ...
                if isnumeric(inOptions) && length(inOptions) == 2
                    [inds, means] = FindStepDWT(trf, inOptions(1),inOptions(2),1);
                else
                    [inds, means] = FindStepDWT(trf, [],[],1);
                end
