function outDwells = BatchFindSteps_Batch(filter, width, decimate, str, inFilePath, fileNames)
    ...
    if ind == size(crops,1)+1
        ...
        for j = 1:length(con)
            ...
            if length(trace) > 100 %dont process empty / tiny traces
                ...
                [inds, means] = AFindSteps(trf,100, 9*estimateNoise(trf,125/decimate) );    %,log(length(trace)));
                dw{j} = diff(inds);
                bu{j} = -diff(means);
            end
        end
