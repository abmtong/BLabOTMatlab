...
%@extrema is plagiarized (/is uncredited) from the file exchange

function out = getstep2(data, base, plotopt) 
% Find maximum and minimum of data
dmin=min(data);
dmax=max(data);
data = data(:);
% base = base(:); %used to be second arg, assuming it's the base signal
...
k=1;
while val<=dmax+ind,
    out(k,1)=val; %#ok<AGROW>
    out(k,2)=sum(data<val); %#ok<AGROW>
    val=val+ind;
    k=k+1;
end
...
% This function analyzes a signal to determine local extrema (value and location)
...
for i=1:length(imax),
    if diffarr(i,2)>dev,
        ...
        if diffarr(i,3)<=dev,
            ...
            peeks1(k)=mean(diffarr(i:(i+j),1)); %#ok<AGROW>
            k=k+1;
        end
    end
end
...
maxfunc=interp1(data(imax),xmax,data(:,1),'pchip');
minfunc=interp1(data(imin),xmin,data(:,1),'pchip');
