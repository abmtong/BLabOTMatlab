...
addpath ([thispath '\ForceExt\'])       %Filename sorter
addpath ([thispath '\StepFind_KV\'])       %Filename sorter
...
stepHist = uicontrol('Parent', pantop, 'Units', 'normalized', 'Position', [.3, 0, .1, 1], 'String', '[]', 'Callback',[]);
...
datTable  = uitable  ('Parent', panlef, 'Units', 'normalized', 'Position', [-.1 .35 1.1 .25], 'Data', zeros(8,1), 'RowName', {'Last' 'PL' 'SM' 'CL' 'Avg' 'PL' 'SM' 'CL'}, 'ColumnName', '');
...
%%%%Callbacks
    function loadFile_callback(~,~, f, p)
        ...
        %Plot sqrt(F)-X on top
        frc = 1./sqrt(frc);
        plotandlim(mainAxis, ext, frc, 'Color', [.7 .7 .7])
        mainAxis.YLim = [0.1 1];
        ...
        if ind
            datTable.Data(1:4) = [1 xwlcData{ind, 2}];
        else
            datTable.Data(1:4) = zeros(1,4);
        end
    end
    ...
    function refilter_callback(~,~)
        ...
        filtLine = surface(subAxis, [extF;extF],[frcF;frcF],zeros(2,length(timF)),[timF;timF], 'edgecol', 'interp');
        filtLine2 = surface(mainAxis, [extF;extF],[frcF;frcF],zeros(2,length(extF)),[timF;timF], 'edgecol', 'interp');
    end
    ...
    function fitXWLC_callback(~,~)
        ...
        ind = find(strcmp(name, xwlcData(:,1)));
        if ind
            xwlcData{ind, 2} = xwlcfit ./ [1 1 1000];
        else %Not found, append to end
            xwlcData(end+1,:) = {name xwlcfit ./ [1 1 1000]};
        end
        
        %Plot residual, fit
        fitx = xwlcfit(3) * .34 * ForceExt_XWLC_Wikipedia(frcFc, xwlcfit(1),xwlcfit(2));
        extresid = extFc - fitx;
        delete(fitLine)
        ...
        dat = dat / (len-1);
        %Update table
        datTable.Data = [0; (xwlcfit ./ [1 1 1000])'; len-1; dat';];
    end
