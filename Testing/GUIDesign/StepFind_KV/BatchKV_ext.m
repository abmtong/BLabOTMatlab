function [outInd, outMea, outTra, stepDist] = BatchKV_ext(inContour, inExt, inPenalty, maxSteps, verbose)
%inContour now a cell array of contours
%Stepfinds over inExt, sizes given by inContour
...
ppool = gcp('nocreate');
if isempty(ppool)
    [outInd, ~, ~] = cellfun(@(x)AFindStepsV4(x, inPenalty, maxSteps, 3), inExt, 'Uni', 0);
else
    parfor i = 1:hei
        [outInd{i}, ~, ~] = AFindStepsV4(inExt{i}, inPenalty, maxSteps, 3);
    end
end
fprintf('\b]\n')

%Calculate step sizes
outMea = cellfun(@ind2mea, outInd, inContour, 'Uni', 0);
outTra = cellfun(@ind2tra, outInd, outMea, 'Uni', 0);
