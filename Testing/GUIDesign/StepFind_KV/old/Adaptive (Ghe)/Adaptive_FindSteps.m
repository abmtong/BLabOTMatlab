function DwellInd = Adaptive_FindSteps(Y,PenaltyFactor)
    ...
    while strcmp(Status,'NotFinished')
        [currSIC DwellInd]= Adaptive_ComputeSIC(Y,StepInd,DwellInd,PenaltyFactor);
        sichist = [sichist currSIC];
        ...
        for i=1:length(Y)-1 %to avoid problems when reaching the last point
            ...
            if sum(StepInd==i)==0 %don't consider points which already correspond to steps
                [tempDwellInd tempStepInd] = Adaptive_UpdateDwellCoordinate(Y,StepInd,DwellInd,i); %try the current point as a putative step location
                [tempSIC(i) tempDwellInd]  = Adaptive_ComputeSIC(Y,tempStepInd,tempDwellInd,PenaltyFactor);
        ...
        if minSIC<currSIC %if the SIC with a step is lower than the SIC witout a step, the step is approved
            ...
            %fprintf('%d|', LastStepDetectedInd)
            [DwellInd StepInd]  = Adaptive_UpdateDwellCoordinate(Y,StepInd,DwellInd,LastStepDetectedInd);
    ...
    [SIC DwellInd] = Adaptive_ComputeSIC(Y,StepInd,DwellInd); %do this to update the DwellInd for the last time
