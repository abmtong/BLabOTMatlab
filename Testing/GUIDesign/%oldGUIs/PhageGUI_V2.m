...
%%%%Callbacks
    ...
    function segSlider_callback(~,~)
        ...
        cla(mainAxis)
        rawLine  = plot(mainAxis,windowFilter(@mean, tim, [], 2),windowFilter(@mean, con, [], 2),'Color',[0.7 0.7 0.7], 'LineWidth', 1);
        %rawLine  = plot(mainAxis,tim,con,'Color',[0.7 0.7 0.7]);
    ...
    function toWorksp_callback(~,~)
        ...
        if ~isempty(cropT);
            ...
            if any(cropind)
                conFc = conF(cropind);
                timFc = timF(cropind);
                assignin('base','guiCcf', conFc);
                assignin('base','guiTcf', conFc);
            end
        end
...
%%%%Helpers
    ...
    function fixLimit_callback(~,~)
        tlim1 = [stepdata.time{num}(1) stepdata.time{num}(end)+eps(1e6)];
        tlim2 = [stepdata.time{1}(1) stepdata.time{end}(end)];
        clim1 = [grabmin(stepdata.contour{num}, stepdata.force{num}), grabmax(stepdata.contour{num}, stepdata.force{num})+eps(1e6)];
        clim2 = [min(cellfun(@grabmin, stepdata.contour, stepdata.force)), max(cellfun(@grabmax, stepdata.contour, stepdata.force))];
        if length(clim1) ~= 2 || ~issorted(clim1)
            clim1 = [0 6000];
        end
        if length(clim2) ~= 2 || ~issorted(clim2)
            clim2 = [0 6000];
        end
        zoom out
        xlim(mainAxis, tlim1)
