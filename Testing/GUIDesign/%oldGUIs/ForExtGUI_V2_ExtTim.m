...
datTable  = uitable  ('Parent', panlef, 'Units', 'normalized', 'Position', [-.1 .35 1.1 .25], 'Data', zeros(8,1), 'RowName', {'Last' 'PL' 'SM' 'CL' 'Off' 'Avg' 'PL' 'SM' 'CL' 'OffX'}, 'ColumnName', '');
...
%%%%Callbacks
    function loadFile_callback(~,~, f, p)
        ...
        %Plot F-T on top
        plotandlim(mainAxis, tim, ext, 'Color', [.7 .7 .7])
    ...
    function refilter_callback(~,~)
        ...
        filtLine = surface(mainAxis, [timF;timF],[extF;extF],zeros(2,length(timF)),[frcF;frcF], 'edgecol', 'interp');
        filtLine2 = surface(subAxis, [extF;extF],[frcF;frcF],zeros(2,length(extF)),[frcF;frcF], 'edgecol', 'interp');
    end
...
%%%%Helpers
    ...
    function custom01_callback(~,~)
        ...
        customB1.String = 'Take PWD';
        %plot PWD in separate window
        a = ginput(2);
        a = a(1:2);
        a = sort(a);
        extcrop = ext(tim>a(1) & tim<a(2) );
        sumPWDV1b(extcrop, 10, .01, 5);
    end

    function custom02_callback(~,~)
        customB2.String = 'AspectRatio';
        xl = mainAxis.XLim;
        yl = mainAxis.YLim;
