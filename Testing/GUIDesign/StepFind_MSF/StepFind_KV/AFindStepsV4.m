...
%Calculate Quadratic Error, sum((mean(A)-A).^2), quickly with @C_qe (MEX file)
%C_qe requires double; data is saved as single

%Speed: Goes as length ^ 2.74 (Only considering @C_qe, theoretical is O(x^2 logx) - assuming QE is O(x^2), which needs to be done log(numSteps) times)
