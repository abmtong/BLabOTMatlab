function out = CalibrateV2(inData, inOpts)
%Takes in normalized data and outputs the calibration values
%V2 : Optimize the optimization, remove old comments
%Todo: Alpha and F3 are covariant, maybe just use one? difference in optimization?
...
%fitfcn = @(x)(Lorentzian(x,Fbf,[opts.Fs opts.nAlias]) - Pbf) .* Sf;
fitfcn = @(x)(log(Lorentzian(x,Fbf,[opts.Fs opts.nAlias])) - log(Pbf));
...
figure
%loglog(Fb(2:end),(Pb(2:end)),'o','Color',[0.8 0.8 0.8])
%hold on
loglog(Fbf,Pbf,'o')
hold on
loglog(Fbf, (Lorentzian(out,Fbf, [opts.Fs opts.nAlias])))
out(3) = (out(3)^2+1)^-.5;
