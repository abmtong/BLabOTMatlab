function [outInd, outMean, outS] = findStepHistV8b(inContour, inYRes, inNoise, p, inDec)
...
dS = zeros(len-1,hei);
histlen = ceil(25/inDec); %80 is a fudged value - seems to fit well through different decimation values
for i = 1:len-1
    
    [beadPos, history] = fsh8_sub1v2 (i, hei, a, histlen, dS);
    
    %Calculate quadratic errors
    
    Q = fsh8_sub2v2 (hei, beadPos, a, history, inContour(i+1))    ;
