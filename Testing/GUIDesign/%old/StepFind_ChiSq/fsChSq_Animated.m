function [outInd, outMean, outTra, fitRatio, allInds] = fsChSq_Animated(inContour, maxSteps, plotFreq )
...
scrsz = scrsz(3:4);
figure('Name','ChSq Animated','Position',[scrsz*.1 scrsz*.8]);
ax = axes;
...
%Create a function that finds the best step to place within [ind1, ind2]. Use nested fcn to access inContour for free.
    function [outInd, outdQE] = findStep(ind1, ind2)
        ...
        for ii = 2:hei-1
            testdQE = C_qe(inContour(ind1    :ind1+ii-1 )) ...
                     +C_qe(inContour(ind1+ii :ind2-1 )) ...
                     -oldQE;
            if outdQE > testdQE
                outdQE = testdQE;
                outInd = ii + ind1-1;
...
for i = 1:maxSteps
    ...
    %Plot correct, xfit with scores
    if ~mod(i, plotFreq)
        cla(ax)
        plot(ax,inContour, 'Color', [.8 .8 .8])
        hold on
        plot(ax, ind3tra(in, inContour), 'b')
        plot(ax, ind3tra(xin, inContour), 'r')
        text(1,10+min(inContour),sprintf('%0.2f Ratio, %0.2f Real, %0.2f Counter', fitRatio(i), rQE, xQE))
        %ginput(1); %Halt exec. by ginput or breakpoint, e.g.
        a=1;
    end
end
endT = toc(startT);
...
if verbose == 1
    ...
    ylim([1, inf]);
elseif verbose == 2
    %verbose == 2, zero out values outside ginput lines
    figure
    plot(fitRatio)
    ylim([1, inf]);
    drawnow
