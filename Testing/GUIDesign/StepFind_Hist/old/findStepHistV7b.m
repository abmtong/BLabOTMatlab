function [outInd, outMean, outS] = findStepHistV7b(inContour, inYRes, inNoise, p)
...
%V7b: Some shortcuts in calculating dS(i,:), slight speedup (<10%)
...
for i = 1:len-1
    ...
    for j = 1:hei
        ...
        if dx <= 30
            %Calculate new score = old score + penalty [+ quadratic error: QE is the same for all j, so add it later]
            [val, ind] = min(J+W(j,:));
