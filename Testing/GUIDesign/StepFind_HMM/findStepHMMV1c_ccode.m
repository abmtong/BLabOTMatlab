function out = findStepHMMV1c_ccode(inTrace, inModel, verbose)
...
%V1c: Adds more info to output file (binsz, maxstep)
...
if nargin < 1 || isempty(inTrace)
    ...
    inTrace = [inTrace];% 8.6+inTrace 8.6*2+inTrace 8.6*3+inTrace];
...
%bin size - how to granularize y
if ~isfield(inModel, 'binsz')
    binsz = 0.1;
else
    binsz = inModel.binsz;
end
...
%max step size
if ~isfield(inModel, 'maxstep')
    maxstep = 15;
else
    maxstep = inModel.maxstep;
end
...
%     a = normpdf(0:binsz:maxstep, guessmean, guesssig);
    ...
    %guess transition prob is 1/1e3
    pstep = 0.001;
...
maxdif = max(abs([diff(tr) maxstep]));
...
if ~exist('pi', 'var')
    pi = npdf2(1);
else %shift pi into bound-index space
    pi2 = zeros(1,hei);
...
tic
newa2 = findStepHMMV1c_ccode_xi(tr, al, a, y, be, lb, ub, wid, sig);
newa2 = newa2 / sum(newa2);
toc

% tic
% newa2 = findStepHMMV1c_ccode_xiV2(tr, al, a, y, be, int32(lb), int32(ub), int32(wid), int32(lb2), int32(maxwid2), sig);
% newa2 = newa2 / sum(newa2);
% toc

tic
newa2m = findStepHMMV1c_ccode_xi_mex(tr, al, a, y, be, int32(lb), int32(ub), int32(wid), sig);
newa2m = newa2m / sum(newa2m);
toc
assert(isequal(newa2, newa2m))
figure, plot(newa), hold on, plot(newa2), set(gca, 'YScale', 'log')

% tic
% newa2 = findStepHMMV1c_ccode_xiV2_mex(tr, al, a, y, be, int32(lb), int32(ub), int32(wid), int32(lb2), int32(maxwid2), sig);
% newa2 = newa2 / sum(newa2);
% toc
...
toc
tic
st2 = findStepHMMV1c_ccode_vitterbi(tr, a, y, sig, lb2, ub2, wid2, maxwid2);
toc
figure, plot(st), hold on, plot(st2)
assert(isequal(st, st2))
%path over each individual likeliest state (max over gamma)
[~, ms] = max(ga,[],2);
ms = ms + lb' -1;

%assemble bayesian filtering path (max over alpha)
[~, msa] = max(al, [], 2);
msa = msa + lb' -1;
