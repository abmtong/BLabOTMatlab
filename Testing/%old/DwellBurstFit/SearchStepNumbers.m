function out = SearchStepNumbers( Contour, Time, StepRange )
...
for n = 1:length(StepRange)
    ...
    %Guess is a stepped line from Time(1),Contour(1) to Time(end),Contour(end) with n steps (2n segments, every other slope 0)
    guessL = ones(1,2*numSteps)*(Time(end)-Time(1))/numSteps/2;
    guessM = vertcat(zeros(1,numSteps),ones(1,numSteps))*(Contour(end)-Contour(1))/(Time(end)-Time(1))*2;
    guessM = reshape(guessM,1,[]);
    guessL = [Time(1) guessL];
    guessM = [Contour(1) guessM];
    guess = [guessL' guessM'];
    %{
