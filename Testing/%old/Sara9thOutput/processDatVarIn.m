...
%The data is stored as a little endian single-precision float (8 bytes)
...
if mod(length(data.Data),numLanes) ~= 0
    fprintf('Warning: data length seems wrong, are you sure numLanes is right?')
end

%Grab each wanted lane 
for i = 1:length(wantedLanes);
    lane = wantedLanes(i);
    %Grab data, fix endianness with @swapbytes
