function modelA(state, pos, ang, outname)
...
motor = bDNA(3.4,0,5,2*pi/5,.7,5, 5*sin(pi/5),0);
...
% for i = 1:5
    ...
    if ~state %0 if pre, 1 if post movement
        cn = surface(xcrot, ycrot, zc+ .34*8.75, .9 * ones(size(zc)));
    else
        cn2 = surface(xcrot, ycrot, zc + .34 * 11.25, .9 * ones(size(zc)));
    end
