function [ATP ADP Nmin MeanT]=MonteCarlo_TestingComplexScenarioB()
% This is roughly the same as MonteCarlo_ScenarioB, except that here we
...
AdpBindingConst      = 100*100; %AdpBindingT      = AdpBindingConst/[ADP] 

ATP   = [1 3 5 8 10 15 25 35 50 100 250 500 1e3 3e3 1e4];
%ATP   = [50];
%ADP   = [0 5 10 20 50 100 200];
ADP=0;
...
for t=1:length(ATP) %t is the ATP concentration index
    ...
    for d=1:length(ADP) %d is the ADP concentration index
        ...
        for s=1:SimulationN %s is the simulation number index
            ...
            T(s) = MonteCarlo_ComplexScenarioB(AdpBindingT,AdpReleaseT,AtpLooseBindingT,AtpLooseReleaseT,AtpTightBindingT,HydrolysisT,Alpha);
        end
