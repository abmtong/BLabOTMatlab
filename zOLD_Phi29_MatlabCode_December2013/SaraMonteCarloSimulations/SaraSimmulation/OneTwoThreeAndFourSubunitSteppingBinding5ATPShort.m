...
k = 10.0;                      % rate (in "real units", s^-1)      %??? why this rate (supposedly the rate of the step is once every 0.5 seconds)
...
NoSteps = 0;                  % Number of Steps to be taken by the motor
...
while t < nTimeSteps,
    ...
    if r1(1,t) < k*dt*(1-(k*dt))% if the random number is small than this value (lambda*time - lambda*time^2) 
        ...
        if ATPs == 5 ;         % Stepping Only happens when 5 molecules bind ATP
            ...
            s = s + 1;            % take a step
            Pos=Pos-Stepsize*NoSteps;   % Increment the position by four step sizes a total of 10 bp per time
