function [Nmin DwellTimes]=MonteCarlo_Simulating5Subunits_withATPgS(ATP,ATPgS,Tatp_on, Tatpgs_on,Tatp_off,Tatpgs_off,Tadp_on,Tadp_off,Tatp_tight,Tatpgs_tight,AlphaT,AlphaD,AlphaTb,Nrounds)
...
ActiveSubunits=5; %here we assume that only 5 subunits are active in one cycle, and one subunit is different from the rest
...
for r=1:Nrounds
    ...
    while n<=ActiveSubunits 
        ...
        else strcmp(Nucleotide, 'ATPgs')
            %disp('entered here')
            CurrentCycleT = exprnd(Tatpgs_off);
