function NewDwells = CompareNewVersusOldDwells(PhageData, OldDwells, NewDwells, BinThr, FeedbackCycle)
% Compare every single "new" dwell against the "old" dwells, keep the old
% dwells if they fit the data better, or vice-versa.
%
% USE: NewDwells = CompareNewVersusOldDwells(PhageData, OldDwells, NewDwells, BinThr, Nmin, FeedbackCycle)
%
...
while i<=length(NewDwells.mean)
    ...
    [OverlapInd, Overlap, Npts, NptsAbove] = FindOverlappingOldDwells(PhageData, OldDwells, NewDwells, i, FeedbackCycle);
    ...
    elseif length(OverlapInd)==1
        ...
        [NewDwells,i] = DealWithOnlyOneOverlap(PhageData,NewDwells,OldDwells,OverlapInd,BinThr,i,FeedbackCycle);
    ...
    elseif length(OverlapInd)>1
        ...
        while k<=length(OverlapInd) && ~strcmp(DwellStatus,'split') %go through the overlapping old dwells, as long as the new dwell hasn't been split
            ...
            if k==1 && sum(Overlap{k})<OldDwells.Npts(OverlapInd(k))
               ...
               %disp('Start Overlap');
               [NewDwells,i,k,DwellStatus]=DealWithStartOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k,FeedbackCycle);
            elseif sum(Overlap{k})==OldDwells.Npts(OverlapInd(k)) && ~strcmp(DwellStatus,'split')
               %we have a complete overlap
               %disp('Middle Overlap');
               [NewDwells,i,k,DwellStatus]=DealWithMiddleOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k,FeedbackCycle);
            elseif k==length(OverlapInd) && sum(Overlap{k})<OldDwells.Npts(OverlapInd(k)) && ~strcmp(DwellStatus,'split')
               ...
               %disp('End Overlap');
               [NewDwells,i,k,DwellStatus]=DealWithEndOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k,FeedbackCycle);
            end
        end
    end
end
