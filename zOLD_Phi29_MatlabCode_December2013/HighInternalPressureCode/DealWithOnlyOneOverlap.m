function [NewDwells,i] = DealWithOnlyOneOverlap(PhageData,NewDwells,OldDwells,OverlapInd,BinThr,i,FeedbackCycle)
...
if OldDwells.Npts(OverlapInd)==NewDwells.Npts(i)
    ...
    data      = PhageData.contourFiltered{FeedbackCycle}(NewDwells.start(i):NewDwells.end(i)); %the extent of the current New Dwell
    ...
    if strcmp(Verdict,'diff')
        ...
        if i~=1 %i==1 doesn't work since there is no previous dwell
            ...
            if ~isempty(OverlapWithPrevDwell) && OverlapWithPrevDwell<NewDwells.Npts(i-1)/2
                ...
                TempCurrNewDwell.mean  = mean(PhageData.contourFiltered{FeedbackCycle}(TempCurrNewDwell.start:TempCurrNewDwell.end));
                TempCurrNewDwell.std   = std(PhageData.contourFiltered{FeedbackCycle}(TempCurrNewDwell.start:TempCurrNewDwell.end));
                TempCurrNewDwell.Npts  = length(PhageData.contourFiltered{FeedbackCycle}(TempCurrNewDwell.start:TempCurrNewDwell.end));
                ...
                TempPrevNewDwell.mean  = mean(PhageData.contourFiltered{FeedbackCycle}(TempPrevNewDwell.start:TempPrevNewDwell.end));
                TempPrevNewDwell.std   = std(PhageData.contourFiltered{FeedbackCycle}(TempPrevNewDwell.start:TempPrevNewDwell.end));
                TempPrevNewDwell.Npts  = length(PhageData.contourFiltered(TempPrevNewDwell.start:TempPrevNewDwell.end));
        ...
        if i~=length(NewDwells.mean) %anything by the very last dwell
            ...
            if ~isempty(OverlapWithNextDwell) && OverlapWithNextDwell<NewDwells.Npts(i+1)/2
                ...
                TempNextNewDwell.mean  = mean(PhageData.contourFiltered{FeedbackCycle}(TempNextNewDwell.start:TempNextNewDwell.end));
                TempNextNewDwell.std   = std(PhageData.contourFiltered{FeedbackCycle}(TempNextNewDwell.start:TempNextNewDwell.end));
                TempNextNewDwell.Npts  = length(PhageData.contourFiltered{FeedbackCycle}(TempNextNewDwell.start:TempNextNewDwell.end));
