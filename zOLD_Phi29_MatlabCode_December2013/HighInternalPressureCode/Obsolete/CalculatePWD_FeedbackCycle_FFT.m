...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for i=1:length(Index)
            ...
            phagePWD(count).hist = n; 
            phagePWD(count).bin = d;
            phagePWD(count).filter = win;
            phagePWD(count).binsize = binsize;
            phagePWD(count).file = phageData(i).file;
            phagePWD(count).stID = phageData(i).stID;
            phagePWD(count).contourRange = index;
            
            %Calculate fft of PWD
            fsamp = 1/binsize;
            nspec = abs(fft((n-mean(n)))).^2;
            freq = (0:length(nspec)-1)*fsamp/(length(nspec)-1);
            df = freq(2)-freq(1);
            phagePWD(count).fft = nspec(1:floor(length(nspec)/2)); 
            phagePWD(count).freq = freq(1:floor(length(nspec)/2));      
            %Sort for peak
            pind = find(phagePWD(count).freq >= 1/10 & phagePWD(count).freq <= 1/8);
            totalpw = sum(phagePWD(count).fft*df);
            peakpw = sum(phagePWD(count).fft(pind)*(df));
            phagePWD(count).peak = peakpw./totalpw;
            count = count + 1;
            pause;
            close(PWDfig);
