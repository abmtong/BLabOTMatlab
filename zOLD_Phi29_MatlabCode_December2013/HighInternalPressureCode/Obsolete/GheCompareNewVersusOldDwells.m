function NewDwells=GheCompareNewVersusOldDwells(PhageData, OldDwells, NewDwells, BinThr, Nmin, FeedbackCycle)
    ...
    elseif length(OverlapInd)>1
        ...
        while k<=length(OverlapInd) && ~strcmp(DwellStatus,'split') %go through the overlapping old dwells, as long as the new dwell hasn't been split
            ...
            if k==1 && sum(Overlap{k})<OldDwells.Npts(OverlapInd(k))
               ...
               %disp('Start Overlap');
               [NewDwells,i,k,DwellStatus]=GheDealWithStartOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k);
            elseif sum(Overlap{k})==OldDwells.Npts(OverlapInd(k)) && ~strcmp(DwellStatus,'split')
               %we have a complete overlap
               %disp('Middle Overlap');
               [NewDwells,i,k,DwellStatus]=GheDealWithMiddleOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k);
            elseif k==length(OverlapInd) && sum(Overlap{k})<OldDwells.Npts(OverlapInd(k)) && ~strcmp(DwellStatus,'split')
               ...
               %disp('End Overlap');
               [NewDwells,i,k,DwellStatus]=GheDealWithEndOverlap(PhageData,NewDwells,OldDwells,OverlapInd,i,k);
            end
        end
    end
end