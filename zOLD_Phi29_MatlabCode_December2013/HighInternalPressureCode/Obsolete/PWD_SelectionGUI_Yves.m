function PWD_SelectionGUI_Yves(fcn)
...
zoom on
   ...
   case 2 % take care of anything else you
    ...
    case 3
        ...
        Dist=[]; %the array that stores all the distaces between points
        %HistBins = min(CropLength):BinPWD:max(CropLength);
        for i=1:length(CropLength);
            Dist=[Dist; abs(CropLength(i+1:end)-CropLength(i))];
        end
        
        %[N, D]   = GhePairWiseDistribution(CropLength,HistBins);
        HistBins=0:BinPWD:range(CropLength);
        [N, D]   = hist(Dist,HistBins);
        %normalize the PWD;
        N=N/sum(N);
        figure;
        %plot(D,N,'r'); 
        hist(Dist,HistBins);
        xlabel('Pairwise Distance (nm)');
        ylabel('Normalized Pairwise Distribution');
     %   zoom reset;
      %  zoom on;
    case 4
        zoom on;
