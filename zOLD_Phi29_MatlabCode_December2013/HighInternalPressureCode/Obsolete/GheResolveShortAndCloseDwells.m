function [Dwells Status] = GheResolveShortAndCloseDwells(phageData, Dwells, Nmin, MinStep)
...
%
% Use: [Dwells Status] = GheResolveShortAndCloseDwells(phageData, Dwells, Nmin, MinStep)
%
% Gheorghe Chistol, May 25, 2010

%% Getting Rid of the Dwells that are Too Close to Each Other
...
while i<=length(Dwells.mean) %go through the updated dwells, merge dwells going forward
    ...
    %Only merge forward, i.e with the next dwell
    ...
    if (DeltaNext<=MinStep) %if the next dwell is too close, proceed to merging
        Status='Merged';
        temp = phageData.contour(Dwells.start(i):Dwells.end(i+1)); 
    ...
    elseif (DeltaPrev<=MinStep) %if the previous step is too close, proceed to merging
        Status='Merged';
        temp = phageData.contour(Dwells.start(i-1):Dwells.end(i)); 
        %temp: contour length data from the trace corresponding to the merged dwell
...
%% Check if the first or the last dwell are very short and get rid of them
%if Dwells.Npts(1)<=Nmin
%    Dwells.start(1) = [];
%    Dwells.end(1) =   [];
%    Dwells.mean(1) =  [];
%    Dwells.std(1) =   [];
