...
for i=1:length(File)
    ...
    if exist(CropFile,'file') %check if the crop file exists, if it doesn't, don't process the phage file at all
        ...
        %K = floor(abs(Contour(1)-Contour(end))/VelWinLength)-1; %how many full velocity windows we can get
        %using a sliding window to sample velocities and smooth over noise
%        figure; hold on;
        StartL=Contour(end); %we start here
        FinishL = StartL+VelWinLength;
        k=1;
        while FinishL < Contour(1)
            ...
            y = Contour(SelectInd);
            Velocity(end+1)=(y(end)-y(1))/(x(end)-x(1));
