...
%         if length(KernelDensity{f}.KernelValue) ~= length(KernelDensity{f}.KernelGrid)
%             keyboard;
%         end
    ...
    ImageFolderName=[analysisPath filesep 'AdaptiveStepFinding_Images_ExtendedRangeForFiltering']; %Save the current figure as an image in a folder for later 
...
%     saveas(gcf,ImageFileName);
    ...
    saveas(gcf,ImageFileName); 
    %ImageFileName = [ImageFolderName filesep 'SIC' PhageFile '_FC' num2str(CurrentFeedbackCycle) '_Sect' num2str(s) '_Band' num2str(Bandwidth) 'Hz' '.fig'];
    %saveas(gcf,ImageFileName); 
    close(gcf);
end