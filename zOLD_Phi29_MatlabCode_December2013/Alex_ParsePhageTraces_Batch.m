function Alex_ParsePhageTraces_Batch(fullFilePath)
...
%
% USE: ParsePhageTraces_Batch()
%
% Gheorghe Chistol, 09 Feb 2012

% Modified by Alex to use a passed file path instead of the gui i/o
...
Prompts = {'fsamp (Hz)','Trap X Conversion (nm/V)', ...
           ...
           'DNA Persistence Length (nm)','DNA Strech Modulus (pN)'};
DefaultParams = {'2500','762','578','1.4','0.9','50','800'}; %Alex 091417
%DefaultParams = {'2500','762','578','1.34','0.43'}; % Ghe, Valid after 05/03/10
%DefaultParams = {'2500','748','565','0.54','1.08'}; % Ghe, Valid for July 31st 2009 - March 22nd 2010

InputParams = str2double(inputdlg(Prompts,'Enter Params for Phage Trace',1,DefaultParams));
...
Params.PersLength   = InputParams(6); %DNA persistence length in nm
Params.StrModulus   = InputParams(7); %DNA stretch modulus, in pN
...
%A Get it from an input arg instead of gui i/o
if(nargin < 1)
    [fileName, filePath, ~] = uigetfile([analysisPath '*.txt']);
    fullFilePath = [filePath fileName];
end
[BatchFilePath, BatchFile, BatchFileExt] = fileparts(fullFilePath);
BatchFile = strcat(BatchFile, BatchFileExt);
...
for f=1:length(RawFile)
    ...
    stepdata = ParsePhageTraces_ProcessOneFile(Files,Params);
