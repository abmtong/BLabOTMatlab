...
% Gheorghe Chistol, 13 Oct 2012
    ...
    FiltBand          = 15; %filtering for finding pauses (important)
    VelFiltBand       = 50; %filtering for calculating force bins (not too important)
        ...
        if ~exist(CropFile,'file') %do not process trace if crop file doesn't exist
            ...
            for fc = 1:length(Trace.time) % 'fc' is the index of the feedback cycle
                ...
                elseif range(Time)<FeedbackCycleTmin 
                    ...
                    [Data(ind).Slips Data(ind).Pauses Data(ind).SlipPauseFreeSegments] = ...
                        CalculateVelocity_RemoveSlipsPauses(Data(ind), MinPauseDur, MinSlipSize);
                    %display(length(Data(ind).Slips));
                    PlotSave.FilePath = [Data(ind).FilePath filesep 'VelocityCalculation' filesep Data(ind).FileName(1:end-4)]; %FilePath is the same as analysisPath
