function [OverlapInd, Overlap, Npts, NptsAbove] = StepFinding_FindOverlappingOldDwells(Data,OldDwells,NewDwells,i)
...
%
% USE: [OverlapInd, Overlap, Npts, NptsAbove] = StepFinding_FindOverlappingOldDwells(Data,OldDwells,NewDwells,i)
%
% Gheorghe Chistol, 15 March 2011

%Create a vector with ones where the new dwell exists and zeros everywhere else
NewVector = zeros(1,length(Data.FilteredContour)); %create zero vector
...
for j=1:length(OldDwells.mean) %find Old Dwells overlapping the New Dwell
    ...
    OldVector = zeros(1,length(Data.FilteredContour)); %create zero vector
    ...
    if sum(temp)>0
        if ~exist('OverlapInd','var') %if Overlap data structure is non-existent, create it
            ...
            data = Data.FilteredContour(start:finish);
