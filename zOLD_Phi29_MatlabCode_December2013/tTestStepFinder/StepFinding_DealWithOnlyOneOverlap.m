function [NewDwells,i] = StepFinding_DealWithOnlyOneOverlap(Data,NewDwells,OldDwells,OverlapInd,BinThr,i)
% This is a script which is part of the
% StepFinding_CompareNewVersusOldDwells function, but I had to take it out
...
if OldDwells.Npts(OverlapInd)==NewDwells.Npts(i)
    ...
    i=i+1;
    %disp('    DealWithOnlyOneOverlap: We have a perfect dwell match');
else
    %The Old Dwell is larger than the New Dwell
    data      = Data.FilteredContour(NewDwells.start(i):NewDwells.end(i)); %the extent of the current New Dwell
    ...
    Verdict   = StepFinding_BinomialVerdict(NptsAbove, Npts, BinThr); %the verdict can either be "diff" or "same"            
    ...
    if strcmp(Verdict,'diff')
        ...
        %disp('    DealWithOnlyOneOverlap: New Dwell is distinct from the old dwell');
    else
        ...
        if i~=1 %i==1 doesn't work since there is no previous dwell
            ...
            if ~isempty(OverlapWithPrevDwell) && OverlapWithPrevDwell<NewDwells.Npts(i-1)/2
                ...
                TempCurrNewDwell.mean  = mean(Data.FilteredContour(TempCurrNewDwell.start:TempCurrNewDwell.end));
                TempCurrNewDwell.std   = std(Data.FilteredContour(TempCurrNewDwell.start:TempCurrNewDwell.end));
                TempCurrNewDwell.Npts  = length(Data.FilteredContour(TempCurrNewDwell.start:TempCurrNewDwell.end));
                ...
                TempPrevNewDwell.mean  = mean(Data.FilteredContour(TempPrevNewDwell.start:TempPrevNewDwell.end));
                TempPrevNewDwell.std   = std(Data.FilteredContour(TempPrevNewDwell.start:TempPrevNewDwell.end));
                TempPrevNewDwell.Npts  = length(Data.FilteredContour(TempPrevNewDwell.start:TempPrevNewDwell.end));
        ...
        if i~=length(NewDwells.mean) %anything by the very last dwell
            ...
            if ~isempty(OverlapWithNextDwell) && OverlapWithNextDwell<NewDwells.Npts(i+1)/2
                ...
                TempNextNewDwell.mean  = mean(Data.FilteredContour(TempNextNewDwell.start:TempNextNewDwell.end));
                TempNextNewDwell.std   = std(Data.FilteredContour(TempNextNewDwell.start:TempNextNewDwell.end));
                TempNextNewDwell.Npts  = length(Data.FilteredContour(TempNextNewDwell.start:TempNextNewDwell.end));
