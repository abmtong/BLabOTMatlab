function [Dwells Status] = StepFinding_ResolveShortAndCloseDwells(Data,Dwells,Nmin,MinStep)
...
%
% Use: [Dwells Status] = StepFinding_ResolveShortAndCloseDwells(Data,Dwells,Nmin,MinStep)
%
...
while i<=length(Dwells.mean) %go through the updated dwells, merge dwells going forward
    ...
    if (DeltaNext<=MinStep) %if the next dwell is too close, proceed to merging
        ...
        if (DeltaPrev<=MinStep) && (DeltaPrev<DeltaNext)
            ...
            Status='Merged';
            temp = Data.FilteredContour(Dwells.start(i-1):Dwells.end(i)); 
            ...
            Status='Merged';
            temp = Data.FilteredContour(Dwells.start(i):Dwells.end(i+1)); 
            %temp: contour length data from the trace corresponding to the merged dwell
