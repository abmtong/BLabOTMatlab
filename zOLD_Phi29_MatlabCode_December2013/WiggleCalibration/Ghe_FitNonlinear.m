function [param, resnorm, r, J, x] = Ghe_FitNonlinear(FitFunct,param,Tolf,MaxIter,varargin)
% [scal_fit,RESNORM] = Ghe_FitNonlinear(FittingFunction,scal_fit,TermTol,MaxIter,parameters0,xfin,yfin,sfin);
%Solves the problem:   min(sum(f(x,y...,param).^2)),  with respect to the parameters in vector 'param'
%                              param 
%
%fit_nonl fits an inline expression or trial function 'f' to the data given in
%the vectors in 'varargin' (see help on 'varargin')
%with initial fitting parameters contained in the vector 'param'.
%When the error decreases less than Tolf during one iteration, the results are
%returned.
...
defaultopt = struct('MaxIter',50,...
                    'Trust_region_initial_value', 2000,...
                    'LineSearchType','Backtracking',...
                    'Method','LevenbergMarquardt',...
                    'call_syntax','[param, resnorm, r, J, x]=Ghe_FitNonlinear(FitFunct,param,Tolf,varargin)'); 

% defaultopt = struct('MaxIter',50,'Trust_region_initial_value', 2000,...
%     'LineSearchType','Backtracking','Method','LevenbergMarquardt','call_syntax',...
%     '[param, resnorm, r, J, x]=fit_nonl(f,param,Tolf,varargin)'); 
...
format short                    
try
    feval(FitFunct,param,varargin{:});
catch
    error('Function cannot be evaluated, try changing your options')
end

if any(isnan(feval(FitFunct,param,varargin{:})))
    param = param +1;   %Try rescue some unlucky input guesses
end

if any(isinf(feval(FitFunct,param,varargin{:})))
    param = param +1;   %Try rescue some unlucky input guesses
end
r = feval(FitFunct,param,varargin{:});
...
while (abs(f0-f1) > Tolf && iter < MaxIter)
    ...
    if (rcond(M) <= eps) disp('Hessian  matrix is close to singular, lambda is increased')
        lambda = lambda_max;    M=H+lambda*eye(m); 
    ...
    pnew = p - dp';
    rnew = feval(FitFunct,pnew,varargin{:});
    f1 = .5*norm(rnew)^2;
    ...
    if (f1 < f0) %everything goes as planned, the function is decreasing
        ...
        [pnew,f1,check] = Ghe_Linesearch(m,p,f0,gradf,-dp,norm(dp),FitFunct,varargin{:});
        pnew = pnew(:)';
        rnew = feval(FitFunct,pnew,varargin{:});
