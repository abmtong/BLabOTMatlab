...
switch Instructions
   case 'Initialize' 
      ...
      uicontrol('Style','Edit','Units','Normalized','Position',[0.0067    0.74    0.0259    0.0402],...
               ...
               'String',3,'BackgroundColor','w','Tag','EditPenaltyFactor');
      uicontrol('Style','Text','Units','Normalized','Position',[-0.009    0.66    0.08    0.0402],...
                'HorizontalAlignment','right','String','Filter Frequency for Kernel Density (Hz):  ',...
                'BackgroundColor',get(gcf,'Color'),'Tag','TextFilterFrequency');
      uicontrol('Style','Edit','Units','Normalized','Position',[0.0067    0.60    0.0259    0.0402],...
               'String','60','BackgroundColor','w','Tag','EditFiltFreqKD');
      ...
      uicontrol('Style','Edit','Units','Normalized','Position',[0.4548    0.9405    0.0259    0.0402],...
                'String','250','BackgroundColor','w','Tag','EditFiltFreq');
    ...
    case 'LoadTrace'
        ...
        display(Trace.FiltFact);
        Trace.FiltFreqKD    = str2double(get(findobj(gcf,'Tag','EditFiltFreqKD'),'String')); %filter frequency for kernel density
        Trace.FiltFactKD    = round(Trace.Bandwidth/Trace.FiltFreqKD); %filter factor for kernel density
        display(Trace.FiltFactKD);
        Trace.Time          = NaN; %initialize the structure that stores data
        Trace.Force         = NaN; %the data for a particular feedback cycle
        Trace.Contour       = NaN;
        Trace.FiltTimeKD    = NaN; % defines new vectors for special kernel density filtering
        Trace.FiltTime      = NaN;
        Trace.FiltForceKD   = NaN; % defines new vectors for special kernel density filtering
        Trace.FiltForce     = NaN;
        Trace.FiltContourKD = NaN; % defines new vectors for special kernel density filtering
    ...
    case 'ComputeNumberofDwells'
        ...
        IndKeepKD = Trace.FiltTimeKD>Trace.LeftBoundT & Trace.FiltTimeKD<Trace.RightBoundT;
        T = Trace.FiltTime(IndKeep);
        Y = Trace.FiltContour(IndKeep);
        F = Trace.FiltForce(IndKeep);
        T_KD = Trace.FiltTimeKD(IndKeepKD);
        Y_KD = Trace.FiltContourKD(IndKeepKD);
        F_KD = Trace.FiltForceKD(IndKeepKD);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    case 'ApplyBoundaries' 
        ...
        Trace.FiltFreqKD = str2double(get(findobj(gcf,'Tag','EditFiltFreqKD'),'String'));
        Trace.FiltFactKD = round(Trace.Bandwidth/Trace.FiltFreqKD); %filter factor 
        [Trace.KernelGrid Trace.KernelValue] = BurstSize_CalculateKernelDensity(CropContour, Trace.FiltFactKD);
        ...
        FilterKD=sprintf('%4.0f', Trace.FiltFreqKD);
        delete(findall(gcf,'Tag','FqPwdKD')) 
        delete(findall(gcf,'Tag','FqPwd')) 
        annotation('textbox', [0.77, 0,0.1,0.1],...
           'String', ['Kernel density was computed at ' FilterKD ' Hz'], 'FontWeight','bold','Color','b','BackgroundColor','none','EdgeColor','none','FontSize',10,'Tag','FqPwdKD');
        guidata(gcf,Trace); %update the gui data
    ...
    case 'SaveResults'
        ...
        SaveFolder = [Trace.FilePath filesep 'BurstDurationAnalysisHeavyEXTRACARFUL'];
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    case 'IdentifyBursts'
        addpath('C:\aPhi29_Files\Phi29_MatlabCode\KernelDensitySpecialFilteringWordWrap'); %to use the wordwrap code
        Trace = guidata(gcf);
...
%                 
        ...
        L  = Pwd_BoxcarFilter(Trace.Contour(PwdRawInd), Trace.FiltFactKD); % the filter for Pairwisefunction is Kernel density filter
        ...
        FilterKD=sprintf('%4.0f', Trace.FiltFreqKD);
        delete(findall(gcf,'Tag','FqPwd'));
        annotation('textbox', [0.1,0.05,0.1,0.1],...
           'String', ['PWD was analyzed at ' FilterKD ' Hz'], 'FontWeight','bold','Color','b','BackgroundColor','none','EdgeColor','none','FontSize',10,'Tag','FqPwd');
