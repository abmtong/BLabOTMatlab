function [ValidatedFragDwellInd KernelDensity] = Adaptive_ValidateDwells_FragDwellInd(RawT,RawY,FiltT,FiltY,FiltF,FragDwellInd,ContrastThr,MaxSeparation,KernelFiltFact,History)
    ...
    for f=1:length(FragDwellInd)
        ...
        [tempDwellInd] = Adaptive_ValidateDwells_FragDwellInd_Consolidate(tempDwellInd,LocalMaxima,MaxSeparation,History);
        
        %Validated Dwells based on the Local Maxima
        [ValidatedFragDwellInd{f} KernelDensity{f}.LocalMaxima] = Adaptive_ValidateDwells_FragDwellInd_Validate(tempDwellInd,LocalMaxima,MaxSeparation); %#ok<AGROW>
