function [DwellInd StepInd] = BurstSize_SIC_UpdateDwellCoordinate(Y,StepInd,DwellInd,i)
...
% DwellInd(d).Var
%
% USE: [DwellInd StepInd] = BurstSize_SIC_UpdateDwellCoordinate(Y,StepInd,DwellInd,i)
%
...
if isempty(StepInd)
    ...
    if i<min(StepInd) %there are no previous steps before the current one
        ...
        DwellInd(1).Mean   = NaN;
        DwellInd(2).Start  = i+1; %where the updated second dwell starts
        DwellInd(2).Mean   = NaN;
    elseif i>max(StepInd) %there are no previous steps after the current one
        ...
        DwellInd(end).Start    = i+1; %this is where the updated last dwell starts
