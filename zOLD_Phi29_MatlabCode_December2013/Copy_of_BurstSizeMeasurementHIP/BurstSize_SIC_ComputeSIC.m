function [SIC DwellInd] = BurstSize_SIC_ComputeSIC(Y,StepInd,DwellInd,PenaltyFactor)
...
%
% USE: [SIC DwellInd] = BurstSize_SIC_ComputeSIC(Y,StepInd,DwellInd)
%                     or
% USE: [SIC DwellInd] = BurstSize_SIC_ComputeSIC(Y,StepInd,DwellInd,PenaltyFactor)
%
...
% Gheorghe Chistol, 29 Dec 2012
    ...
    for d=1:length(DwellInd)
        ...
        len = DwellInd(d).Finish - DwellInd(d).Start;
        if len == 0
            len = 1;
        end
        Var = Var+DwellInd(d).Var/len; %add the current variance to the total variance
    ...
    n   = length(Y);
    k   = length(StepInd);
    SIC = PenaltyFactor*(k+2)*log(n)+n*log(Var);
end