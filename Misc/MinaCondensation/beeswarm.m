...
%   corral_style    ('none' default | 'gutter' | 'omit' | 'random')
...
chanwid = .9; % percent width of channel to use -- Default 0.9, edited
...
if isfinite(p.Results.dot_size)
    ...
    for i=1:length(ux)
        ...
        switch lower(p.Results.overlay_style)
            ...
            case 'mad' %ed by me - median +- mad
                line([1 1]*ux(i),median(yorig(ic==i))+[-1 1]*mad(yorig(ic==i),1),'Color',cmap(i,:),'LineWidth',2)
                line([ux(i)-2*rwid ux(i)+2*rwid],[1 1]*median(yorig(ic==i)),'LineWidth',3,'Color',cmap(i,:))
            case 'ci'
