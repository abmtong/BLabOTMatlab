function rip_data = LA_unfold_finder_AT()

%% Edited for use with Alex tools.
...
rip_data = cell(numel(f), 1);

%% Load the data
for k = 1:numel(f);
%     disp(['Processing ' f{k}]);
    cd = load(fullfile(p, f{k}));
    ContourData = cd.ContourData;
    %This originally loaded .... [pdata] [ContourData]
