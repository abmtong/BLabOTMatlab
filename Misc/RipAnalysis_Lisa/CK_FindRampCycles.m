function [RLegs,ULegs] = CK_FindRampCycles_AT(t,x,fS,Boundaries)
...
if D1(1) < D2(1)
    ...
    if numel(D1)>numel(D2)
        ...
        RLegs(end,2) = numel(x);
        for i = 1:numel(D1)-1
            [junk,RLegs(i,2)]   = min(x(D1(i):D2(i)));
            RLegs(i,2)   = RLegs(i,2)   + D1(i);

            [junk,RLegs(i+1,1)] = max(x(D2(i):D1(i+1)));
        ...
        for i = 2:numel(D1)
            [junk,RLegs(i,1)] = max(x(D2(i-1):D1(i)));
