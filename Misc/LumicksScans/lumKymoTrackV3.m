...
%Crop method
opts.cropmeth = 1; %1: Interactive, 2: auto
        %Auto looks worse, having the close crop is better
opts.autocrop = 30; %cropmeth == 2: Crop this many pixels from top and bottom

%Orient method
opts.orientmeth = 1; %0: Don't do anything, 1: 'guesses' tloc dir.
...
opts.saveoutput = 1; %Save a lumKymo*.mat file
...
if nargin < 1 || isempty(infp)
    ...
    out = cellfun(@(x) lumKymoTrackV3(fullfile(p,x), opts), f, 'Un', 0);
    out = [out{:}];
    return
end
...
%         ax.CLim(2) = ax.CLim(2) /2;
        ...
        close(fg);
    case 2
        %Just crop top/bottom edges
        img = img( 1+opts.autocrop: end-opts.autocrop , :);
end
...
%Orient extension
switch opts.orientmeth
    case 1
        %Guess tloc dir by first 10% vs last 10% position, simple greater than/less than
        %Polyfit? 
        xn = ext(~isnan(ext));
        x1 = median(xn(1: round(end/10) ) );
        x2 = median( xn( end-round(end/10):end) );
        if x1 > x2
            ext = -ext;
        end
        
        %Zero
        ext = ext - prctile(ext, 1);
    otherwise
        %Do nothing
end
...
if opts.saveoutput
    ...
    stepdata.time = {tt};
    stepdata.force = {frc}; %#ok<STRNU>
    save( fullfile(p, ['lumKymo' f e]) , 'stepdata')
end
...
if opts.verbose
    figure('Name', sprintf('lumKymoTrack results for %s', f))
    subplot(3,1,[1 2])
    hold on
    plot(out.time, out.con)
    xlabel('Time (s)')
    ylabel('Transcription (bp, rel.)')
    subplot(3,1,3)
    hold on
