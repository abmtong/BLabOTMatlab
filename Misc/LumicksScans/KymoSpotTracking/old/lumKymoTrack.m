function out = lumKymoTrack(infp, inOpts)
...
if nargin < 1 || isempty(infp)
    ...
    out = cellfun(@(x) lumKymoTrack(fullfile(p,x), opts), f, 'Un', 0);
    return
end
...
%             newtmp(in1(mi):in2(mi))=tmpraw(in1(mi):in2(mi));
            ...
            %Get centroid
            out(i) = sum(yy .* newtmp)/sum(newtmp);
    end
end
...
if opts.debug
    ...
    colormap jet
    %Plot trace over it
    plot(out / opts.pxsz, 'r', 'LineWidth', 2)
end
