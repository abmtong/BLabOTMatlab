function [out, outraw] = RPp6V3(inst, dir)
...
for i = 1:nboot
    ...
    ff2boot = @(x) fitfcn2(pd,rd,x);
    ffboot{i} = fminsearch(ff2boot, xg);
end
ffboot = reshape([ffboot{:}],2,[])';
ft = [ft; mean(ffboot,1); std(ffboot,1)];

%Create function to create unfolding distribution from x0,x
    function out = ffcf(x0,x) %[k0, delta], pull ff's
        %Calculate k
        ks = x0(1) * exp(dir * x * x0(2) / kT); %k = k0 exp(Fd/kT)
        ...
        out = [1 cumprod(unfchc(1:end-1))].*(1-unfchc);
        
        %Normalize
        out = out / abs(sum( (out(1:end-1) + out(2:end))/2 .* diff(x) ) );
    end
...
lb = [0 0];
ub = [inf inf];
optop2 = optimoptions('lsqcurvefit', 'Display', 'off');
[ftcf, ~, rsd, ~, ~, ~, jac] = lsqcurvefit(@ffcf, xg, ff, ppcf, lb, ub, optop2 );
