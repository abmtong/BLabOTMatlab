function out = RPp6V2(inst, inOpts)
...
%    pf = polyfit(tcr, frcf, 1);
   ...
   %Get trap loading rate of pull
   tpos = windowFilter(@mean, double(inst(i).tpos(1:inst(i).ripind)), [], fil);
   tpos = tpos(1:end-endtrim);
   tcr = (1:length(tpos))*fil/Fs;
   pf = polyfit(tcr, tpos, 1);   
   %Calculate trapk too, then can calculate force loading rate
   ext = windowFilter(@mean, double(inst(i).ext(1:inst(i).ripind)), [], fil);
...
%     ff = [fdat{mini} 0 0 0 0 0]; %Add a zeroes at the end to force folding
    ...
    tt = tmax*.5:dtp:tmax; %How far to go, trap pos wise? Say half? We want until F=3 or so, the limit ish of our detection
...
%     end
    ...
    %HACK: Start at f==2pN. Having too many pts at low force = get... wrong? Would-be-invisible? events
%     fminsim = 2;
%     ff = ff(ff>fminsim);
    
    %Calculate chance of not unfolding at each force, = exp(-kt)
    ks = out(2,1) * exp(dir * ff * out(2,2) / kT); %k = k0 exp(Fd/kT)
