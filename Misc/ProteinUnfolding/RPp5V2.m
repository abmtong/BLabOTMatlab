function out = RPp5V2(inst, inOpts)
%Calculates work by method described in doi.org/10.1038/nature04061


opts.fil = 100; %Filter amount. Maybe do a mix of mean + median filtering?
...
opts.hbinsd = [20 20]; %Final histogram sd for kdf for [unfolding, refolding]; pNnm
...
for i = 1:nfile
    ...
    for j = 1:hei
        ...
        yint = cell(1,2); %Save interp'd ys here
...
%             end
        ...
        xpolyd = XWLC(fpoly, tmp.xwlcft(1),  tmp.xwlcft(2)) *  tmp.xwlcft(3); 
        
        %Restrict to x range by interp
        ypoly = interp1( xpoly, fpoly, xxi, 'linear' );
        ypolyd= interp1( xpolyd, fpoly, xxi, 'linear' );
        
        if opts.debug
            plot(xxi + j*dx, ypoly)
            plot(xxi + j*dx, ypolyd)
        end
        ...
        %Add back the portion before xxi(1)
        flo = linspace(.01, opts.frng(2), 1e4);
        xlo = XWLC(flo, tmp.xwlcft(1),  tmp.xwlcft(2)) *  tmp.xwlcft(3) + XWLC(flo, tmp.xwlcft(6),  inf) *  tmp.xwlcft(7); 
        xlod= XWLC(flo, tmp.xwlcft(1),  tmp.xwlcft(2)) *  tmp.xwlcft(3); 
        xlorng = fliplr(xrng(1):-opts.ibinsz:0);
        
        %Interp, out-of-bounds use NaN
        xx = interp1(xlo , flo, xlorng, 'linear', nan);
        xxd= interp1(xlod, flo, xlorng, 'linear', nan);
        
        %And add back
        wrklo = sum(xx - xxd, 'omitnan') * opts.ibinsz;
        wrks(j,:) = wrks(j,:) + wrklo;
        
        %Actually lets crop the rip part
        if opts.croprip
            %Subtract the data and the XWLC fit
            yrip = yint{1} - ypoly;
            yfol = yint{2} - ypoly;
            
            %Find the largest portion above the curve
            yriptf = yrip > 0;
            yfoltf = yfol > 0;
            
            [in, me] = tra2ind(yriptf);
            df = diff(in);
            incrp = in(me == 1);
            [~, maxi] = max(df( me == 1 ));
            %Set to true
            yriptf(1:incrp(maxi)) = true;
            
            [in, me] = tra2ind(yfoltf);
            df = diff(in);
            incrp = in(me == 1);
            [~, maxi] = max(df( me == 1 ));
            %Set to true
            yfoltf(1:incrp(maxi)) = true;
            
            %Crop to the next point that falls below the XWLC fit
            
            ircrp = find(~yriptf, 1, 'first');
            ifcrp = find(~yfoltf, 1, 'first');
            
            wrks(j,1) = sum(yrip(1:ircrp)) * opts.ibinsz + wrklo;
            wrks(j,2) = sum(yfol(1:ifcrp)) * opts.ibinsz + wrklo;
            
            %Add vertical lines for crossing times
            if opts.debug
                plot( xxi(ircrp) * [1 1]  + j*dx, opts.frng)
                plot( xxi(ifcrp) * [1 1]  + j*dx, opts.frng)
            end
