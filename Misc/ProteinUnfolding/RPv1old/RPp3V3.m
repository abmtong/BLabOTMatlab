function out = RPp3V3(inst, inOpts)
...
%V3: Use @fitProteinPull instead. Somewhat slower... maybe do a different way
...
for i = 1:len
    ...
    %Fit piecewise with fitProteinPull:
    % Assemble fit functions, DNA WLC and DNA+Protein WLC
    fh1 = @(x0,f)( x0(3) * XWLC(f-x0(5), x0(1),x0(2)) + x0(4) );
    fh2 = @(x0,f)( x0(3) * XWLC(f-x0(5), x0(1),x0(2)) + x0(4) + x0(7) .* XWLC(f-x0(5), x0(6),inf) );
    
    %Split x/f into the two regions, DNA and DNA+Protein
    xx1 = x(1:ri);
    ff1 = f(1:ri);
    xx2 = [x(ri+1:end) xr];
    ff2 = [f(ri+1:end) fr];
    
    %Create guess, lb, ub structs
    %Fit params are [dPL dSM dCL xOff fOff pPL pCL] ; not using x or f offsets
    xg = { [opts.dwlcg opts.dwlcc 0 0], [nan nan nan 0 0 opts.pwlcg opts.pwlcc] };
    lb = zeros(1,7);
    ub = [inf inf inf 0  0 inf inf];
    
    pft = fitProteinPull({fh1 fh2}, xg, {ff1 ff2}, {xx1 xx2}, lb, ub);
