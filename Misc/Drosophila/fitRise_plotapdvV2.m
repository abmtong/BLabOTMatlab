function fitRise_plotapdvV2(inst, ncs)
...
for j = 1:nnc
    ...
    ki = arrayfun( @(x) (ncs(j) == x), nc );
    end
    fr = fitr(ki);
    
    %Get data from fitRise structs
    ap = cellfun(@(y) arrayfun(@(x) x.cenapdv(1), y), fr, 'Un', 0);
    df = cellfun(@(x) [x.dt], fr, 'Un', 0);
...
%N plot
nn = cellfun(@(x) cellfun(@length, x), rawdat, 'Un', 0);
nn = reshape([nn{:}], [], nnc);


ax2 = subplot(2,1,2);
bs = bar(xcent, nn );
%Color the same way, i.e. make the bar faces the matlab default plot color order
ls = lines(7);
for i = 1:nnc
    ii = mod(i-1,7)+1;
	bs(i).FaceColor = ls(ii,:);
end
...
lgn = arrayfun(@(x,y) sprintf('nc%d, N=%d', x, y), ncs, sum(nn,1), 'Un', 0);
% lgn{1} = 'All ncs';
