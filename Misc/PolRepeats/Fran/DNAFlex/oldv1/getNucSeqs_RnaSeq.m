function out = getNucSeqs_RnaSeq(chrseq, nucpos, inOpts)
...
%Handle batch
if iscell(nucpos)
    out = cellfun(@(x) getNucSeqs_RnaSeq(chrseq, x, opts), nucpos, 'Un', 0);
    return
end
...
for i = nchr:-1:1
    ...
    genmaptpm = nan(size(seq)); %'gene map TPM' [RNA-Seq channel]
    for j = 1:ngene
