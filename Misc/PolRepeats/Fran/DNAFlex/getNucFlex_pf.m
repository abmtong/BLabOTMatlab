function out = getNucFlex_pf(chr, nucpos, inOpts)
...
% opts.ncore = 8; %N cores for parfor
...
%Remove mito DNA
chr( strcmp( {chr.chr}, 'chrM' ) ) = [];
nchr = length(chr);

%First find corresponding nucpos index to chr index
chrind = nan(1, nchr);
for i = 1:nchr
    ...
    chrind(i) = ii;
    %So chr(i) corresponds to nucpos(i)
end

%Rearrange nucpos so that it matches
nucpos = nucpos(chrind);

out = cell(1,nchr);
% parfor (i = 1:nchr, opts.ncore)
parfor i = 1:nchr
    %Flexibility profile, as predicted cyclizibility
    cyc = chr(i).cyc;
    
    npos = nucpos(i).nucpos;
    ...
    for j = 1:nnuc
        switch opts.method
            case 1 %Pad on both sides of a fixed NPS length
                try
                    outcyc{j} = cyc( nucpos(i).nucpos(j,1) - opts.pad : nucpos(i).nucpos(j,2) + opts.pad );
%                     nucloc(j) = round( (nucpos(i).nucpos(j,1)+nucpos(i).nucpos(j,2)) /2);
                catch %Instead of checking for boundaries, just skip ones near the edges
                    fprintf('Skipped nuc on %s on %d-%d\n', nucpos(i).chr, nucpos(i).nucpos(j,1), nucpos(i).nucpos(j,2))
                    nskip = nskip + 1;
                    continue
                end
            case 2 %Pad to a fixed even length
                %Get the center of the sequence, use floor() for halves
                ctr = floor( mean( nucpos(i).nucpos(j,:) )+0.5 ); %e.g. [1, 2] gives 2, [1, 3] gives 2
...
%                 nucloc(j) = ctr;
            ...
            case 3 %Pad to a fixed odd length
                ...
                ctr = floor( mean( nucpos(i).nucpos(j,:) ) ); %e.g. [1, 1] gives 2, [1, 3] gives 2
                %Get the left and right edges
                st = ctr - wid;
                en = ctr + wid;
                %Let's just skip ones close to the sequence edges...
...
%         isp = max( genmapp( nucpos(i).nucpos(j,1) : nucpos(i).nucpos(j,2) ) );
%         ism = max( genmapm( nucpos(i).nucpos(j,1) : nucpos(i).nucpos(j,2) ) );
...
%         try
%             gentpm(j) = genmaptpm( round( (nucpos(i).nucpos(j,1) + nucpos(i).nucpos(j,2))/2 ) );
%         catch
%             %Failed, just skip
%             %Maybe warn here
%         end
