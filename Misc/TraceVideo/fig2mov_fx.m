function fig2mov_fx(infig, nframe)
...
%Delete, going through the traces
nn = length(ob);
frperobj = floor( nframe / nn );
%Reset nframe
% nframe = nn * frperobj;


%Create frames in reverse, so we can continuously remove pts
for i = 1:nn %Go in obj order = last plotted first
    tmp = ob(i);
    len = length( tmp.XData );
    for j = frperobj:-1:1
        %Delete fraction of trace
        imx = floor(len *( j/frperobj ) );
        tmp.XData = tmp.XData (1:imx);
        tmp.YData = tmp.YData (1:imx);
        
        %Save png
        print(fg, sprintf(sfstr, outname,'fig2mov', (nn-i)*frperobj + j),'-dpng',sprintf('-r%d',96*scale))
    end
    
    %Make sure ith object is fully gone
    delete(tmp);
end
